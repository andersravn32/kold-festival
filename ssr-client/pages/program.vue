...
//Load type concert and talk
artists.getArtists(['concert', 'talk']);

const artistDates = ref(artists.data.value.map((artist) => {
    return {
      ...artist,
...
/*   //Creating dataLayer if its doesn't exist
  ...
  if (!artistCols.value.includes(artist.dateShort.substring(0, artist.dateShort.length - 6))) 
    {
  ...
  if (!talkCols.value.includes(artist.dateShort.substring(0, artist.dateShort.length - 6))) 
    {
      talkCols.value.push(
        artist.dateShort.substring(0, artist.dateShort.length - 6)
      );
        ...
        <Tabs v-if="true">
          ...
          <Tab title="Musik" class="tabStyle">
            ...
            >
              ...
              <ul
              class="flex flex-col space-y-2 divide-y-2 divide-blue-900"
            >
